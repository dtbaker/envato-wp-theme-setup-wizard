...
if( ! class_exists('Envato_Theme_Setup_Wizard') ){
    ...
    class Envato_Theme_Setup_Wizard {
    	...
    	public function admin_menus() {
    		add_theme_page(__('Setup Wizard','envato_wizard'), __('Setup Wizard','envato_wizard'), 'manage_options', $this->theme_name.'-setup', array($this,'setup_wizard') );
    	}
    	...
    	public function init_wizard_steps() {
        	...
        	$this->steps = array(
    			'introduction' => array(
    				'name'    =>  __( 'Introduction', 'envato_wizard' ),
    	...
    	public function setup_wizard_header() {
    		...
    		<head>
    			...
    			<title><?php _e( 'Theme &rsaquo; Setup Wizard', 'envato_wizard' ); ?></title>
    	...
    	public function setup_wizard_footer() {
    		?>
    			...
    			<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'Return to the WordPress Dashboard', 'envato_wizard' ); ?></a>
    			<?php endif; ?>
    		</body>
    		<?php
    	...
    	public function envato_setup_introduction() {
    		if(isset($_REQUEST['export'])){
    			...
    			?>
    			<h1><?php _e( 'Welcome to the setup wizard for Beautiful!', 'envato_wizard' ); ?></h1>
    			<p><?php _e( 'Thank you for choosing the Beautiful theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. <br/>It should only take 5 minutes.', 'envato_wizard' ); ?></p>
    			<p><?php _e( 'No time right now? If you don’t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'envato_wizard' ); ?></p>
    			<p class="envato-setup-actions step">
    				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
    				   class="button-primary button button-large button-next"><?php _e( 'Let\'s Go!', 'envato_wizard' ); ?></a>
    				<a href="<?php echo esc_url( wp_get_referer() ? wp_get_referer() : admin_url( '' ) ); ?>"
    				   class="button button-large"><?php _e( 'Not right now', 'envato_wizard' ); ?></a>
    			</p>
    			<?php
    		}
    	}
    	...
    	public function envato_setup_default_plugins() {
    		...
    		?>
    		<h1><?php _e( 'Default Plugins', 'envato_wizard' ); ?></h1>
    		<form method="post">
    			...
    			?>
    				<p><?php _e( 'Your website needs a few essential plugins. The following plugins will be installed:', 'envato_wizard' ); ?></p>
    				...
    				echo '<p><strong>'._e('Good news! All plugins are already installed and up to date. Please continue.','envato_wizard').'</strong></p>';
    			} ?>

    			<p><?php _e( 'You can add and remove plugins later on from within WordPress.' ); ?></p>

    			<p class="envato-setup-actions step">
    				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php _e( 'Continue', 'envato_wizard' ); ?></a>
    				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'envato_wizard' ); ?></a>
    	...
    	public function ajax_plugins(){
    		if(!check_ajax_referer( 'envato_setup_nonce', 'wpnonce') || empty($_POST['slug'])){
    			wp_send_json_error(array('error'=>1,'message'=>__('No Slug Found','envato_wizard')));
    		}
    		$json = array();
    		...
    		foreach($plugins['activate'] as $slug => $plugin){
    			if($_POST['slug'] == $slug){
    				$json = array(
    					...
    					'action2' => -1,
    					'message' => __('Activating Plugin','envato_wizard'),
    				);
    				break;
    			}
    		}
    		foreach($plugins['update'] as $slug => $plugin){
    			if($_POST['slug'] == $slug){
    				$json = array(
    					...
    					'action2' => -1,
    					'message' => __('Updating Plugin','envato_wizard'),
    				);
    				break;
    			}
    		}
    		foreach($plugins['install'] as $slug => $plugin){
    			if($_POST['slug'] == $slug){
    				$json = array(
    					...
    					'action2' => -1,
    					'message' => __('Installing Plugin','envato_wizard'),
    				);
    				break;
    			}
    		}
    		...
    		if($json){
    			...
    			wp_send_json(array('done'=>1,'message'=>__('Success','envato_wizard')));
    		}
    		exit;
    	...
    	private function _content_default_get(){
    		...
    		$content['pages'] = array(
    			'title' => __( 'Pages', 'envato_wizard' ),
    			'description' => __( 'This will create default pages as seen in the demo.', 'envato_wizard' ),
    			'pending' => __( 'Pending.', 'envato_wizard' ),
    			'installing' => __( 'Installing Default Pages.', 'envato_wizard' ),
    			'success' => __( 'Success.', 'envato_wizard' ),
    			'install_callback' => array($this,'_content_install_pages'),
    		);
    		$content['products'] = array(
    			'title' => __( 'Products', 'envato_wizard' ),
    			'description' => __( 'Insert default shop products and categories as seen in the demo.', 'envato_wizard' ),
    			'pending' => __( 'Pending.', 'envato_wizard' ),
    			'installing' => __( 'Installing Default Products.', 'envato_wizard' ),
    			'success' => __( 'Success.', 'envato_wizard' ),
    			'install_callback' => array($this,'_content_install_products'),
    		);
    		$content['widgets'] = array(
    			'title' => __( 'Widgets', 'envato_wizard' ),
    			'description' => __( 'Insert default sidebar widgets as seen in the demo.', 'envato_wizard' ),
    			'pending' => __( 'Pending.', 'envato_wizard' ),
    			'installing' => __( 'Installing Default Widgets.', 'envato_wizard' ),
    			'success' => __( 'Success.', 'envato_wizard' ),
    			'install_callback' => array($this,'_content_install_widgets'),
    		);
    		$content['menu'] = array(
    			'title' => __( 'Menu', 'envato_wizard' ),
    			'description' => __( 'Insert default menu as seen in the demo.', 'envato_wizard' ),
    			'pending' => __( 'Pending.', 'envato_wizard' ),
    			'installing' => __( 'Installing Default Menu.', 'envato_wizard' ),
    			'success' => __( 'Success.', 'envato_wizard' ),
    			'install_callback' => array($this,'_content_install_menu'),
    		);
    		$content['settings'] = array(
    			'title' => __( 'Settings', 'envato_wizard' ),
    			'description' => __( 'Configure default settings.', 'envato_wizard' ),
    			'pending' => __( 'Pending.', 'envato_wizard' ),
    			'installing' => __( 'Installing Default Settings.', 'envato_wizard' ),
    			'success' => __( 'Success.', 'envato_wizard' ),
    	...
    	public function envato_setup_default_content() {
    		?>
    		<h1><?php _e( 'Default Content', 'envato_wizard' ); ?></h1>
    		<form method="post">
    			<p><?php printf( __( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue.', 'envato_wizard' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
    			<table class="envato-setup-pages" cellspacing="0">
    				<thead>
    				<tr>
    					<td class="check"> </td>
    					<th class="item"><?php _e( 'Item', 'envato_wizard' ); ?></th>
    					<th class="description"><?php _e( 'Description', 'envato_wizard' ); ?></th>
    					<th class="status"><?php _e( 'Status', 'envato_wizard' ); ?></th>
    				</tr>
    				</thead>
    				<tbody>
    			...
    			<p><?php _e( 'Once inserted, this content can be managed from the WordPress admin dashboard.' ); ?></p>

    			<p class="envato-setup-actions step">
    				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php _e( 'Continue', 'envato_wizard' ); ?></a>
    				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'envato_wizard' ); ?></a>
    				<?php wp_nonce_field( 'envato-setup' ); ?>
    			</p>
    		</form>
    	...
    	public function ajax_content(){
    		...
    		if(!check_ajax_referer( 'envato_setup_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])){
    			wp_send_json_error(array('error'=>1,'message'=>__('No content Found','envato_wizard')));
    		}
    		...
    		if($json){
    			...
    			wp_send_json(array('error'=>1,'message'=>__('Error','envato_wizard')));
    		}
    ...
    //                    print_r($widget_positions + $import_widget_positions);exit;
    	...
    	private function _get_json($file){
    		if(is_file(__DIR__.'/content/'.basename($file))) {
    			return json_decode( file_get_contents( __DIR__ . '/content/' . basename( $file )), true  );
    		}
    		return array();
    	}
    	...
    	public function envato_setup_logo_design() {
    		...
    		?>
    		<h1><?php _e( 'Logo &amp; Design', 'envato_wizard' ); ?></h1>
    		<form method="post">
    			<p><?php echo sprintf (__('Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.' ,'envato_wizard' ), '<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">','</a>'); ?></p>
    			...
    			<table>
    				<tr>
    					...
    					<td>
    						<a href="#" class="button button-upload"><?php _e( 'Upload New Logo', 'envato_wizard' ); ?></a>
    					</td>
    				</tr>
    			</table>


    			<p><?php _e('Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.' ,'envato_wizard' ); ?></p>
    			...
    			<p class="envato-setup-actions step">
    				<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'envato_wizard' ); ?>" name="save_step" />
    				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'envato_wizard' ); ?></a>
    				<?php wp_nonce_field( 'envato-setup' ); ?>
    			</p>
    		</form>
    	...
    	public function envato_setup_updates() {
    		?>
    		<h1><?php _e( 'Theme Updates', 'envato_wizard' ); ?></h1>
    		...
    		<form method="post">
    			...
    			if(count($my_items)){
    				...
    				<p class="envato-setup-actions step">
    					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next button-primary"><?php _e( 'Continue', 'envato_wizard' ); ?></a>
    				</p>
    				<?php
    			}else{
    			?>
    				<p><?php _e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'envato_wizard' ); ?></p>
    				<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
    				<p>
    				...
    				<p class="envato-setup-actions step">
    					<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Login with Envato', 'envato_wizard' ); ?>" name="save_step" />
    					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'envato_wizard' ); ?></a>
    					<?php wp_nonce_field( 'envato-setup' ); ?>
    				</p>
    			<?php } ?>
    	...
    	public function envato_setup_customize(){
    		...
    		<p class="envato-setup-actions step">
    			<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Continue', 'envato_wizard' ); ?></a>
    		</p>
    	...
    	public function envato_setup_help_support(){
    		...
    		<p class="envato-setup-actions step">
    			<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Agree and Continue', 'envato_wizard' ); ?></a>
    			<?php wp_nonce_field( 'envato-setup' ); ?>
    		</p>
    		<?php
    	}
    	...
    	public function envato_setup_ready() {
    		...
    		<h1><?php _e( 'Your Website is Ready!', 'envato_wizard' ); ?></h1>
    		...
    		<div class="envato-setup-next-steps">
    			<div class="envato-setup-next-steps-first">
    				<h2><?php _e( 'Next Steps', 'envato_wizard' ); ?></h2>
    				<ul>
    					<li class="setup-product"><a class="button button-primary button-large" href="https://twitter.com/dtbaker" target="_blank"><?php _e( 'Follow @dtbaker on Twitter', 'envato_wizard' ); ?></a></li>
    					<li class="setup-product"><a class="button button-next button-large" href="<?php echo esc_url( home_url() ); ?>"><?php _e( 'View your new website!', 'envato_wizard' ); ?></a></li>
    				</ul>
    			</div>
    			<div class="envato-setup-next-steps-last">
    				<h2><?php _e( 'More Resources', 'envato_wizard' ); ?></h2>
    				<ul>
    					<li class="documentation"><a href="http://dtbaker.net/envato/documentation/" target="_blank"><?php _e( 'Read the Theme Documentation', 'envato_wizard' ); ?></a></li>
    					<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php _e( 'Learn how to use WordPress', 'envato_wizard' ); ?></a></li>
    					<li class="rating"><a href="http://themeforest.net/downloads" target="_blank"><?php _e( 'Leave an Item Rating', 'envato_wizard' ); ?></a></li>
    					<li class="support"><a href="http://dtbaker.net/envato/" target="_blank"><?php _e( 'Get Help and Support', 'envato_wizard' ); ?></a></li>
    				</ul>
    			</div>
    		</div>
    		<?php
    	...
    	public function envato_market_admin_init(){
    		...
    		if ( !empty( $_POST['oauth_session'] ) && !empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
    			...
    			if ( is_wp_error( $response ) ) {
    				...
    				$class = "error";
    				echo "<div class=\"$class\"><p>".sprintf(__('Something went wrong while trying to retrieve oauth token: %s','envato_wizard'), $error_message)."</p></div>";
    			} else {
    		...
    		add_settings_section(
    			...
    			sprintf( __( 'Login for %s updates', 'envato-market' ), $this->envato_username  ),
    	...
    	public function render_oauth_login_description_callback() {
    		echo 'If you have purchased items from dtbaker on ThemeForest or CodeCanyon please login here for quick and easy updates. Items include: xxxx yyyy zzzz';
    	...
    	public function render_oauth_login_fields_callback() {
    		...
    		<div class="oauth-login" data-username="<?php echo esc_attr( $this->envato_username ); ?>">
    			<a href="<?php echo esc_url( $this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '#settings' ) ) ); ?>"
    			   class="oauth-login-button button">Login with Envato</a>
    		</div>
    		<?php
    	}
